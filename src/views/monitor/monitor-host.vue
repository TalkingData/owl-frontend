      ...
      <div class="table-list host-list">
        <div class="table-list-header clearfix mb-10">
          <div class="float-left" v-if="isProduct">
            <Button :disabled="!disableObj.isRemove" @click="removeData('multiple')" type="primary" icon="minus">移除主机</Button>
          ...
          <paging :total="total" @on-page-info-change="pageInfoChange" ref="page">
            <div slot="listTable" class="box-content-body" v-if="dataList.length > 0">
              <Row class="box-content-item" v-for="(item, index) in dataList" :key="index" >
                ...
                <Checkbox v-model="item.checked" v-if="isProduct" @click.native.stop="selectItem(item, index)"></Checkbox>
                <a :title="item.name || item.hostname" href="javascript:;" @click="viewDetail(item, index)">
                ...
                <div class="float-right pr-20">
                  <Tooltip content="移除" placement="top" class="ml-10" v-if="isProduct" >
